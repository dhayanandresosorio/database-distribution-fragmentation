...
```sql
ALTER ROLE postgres WITH PASSWORD 'D****a';
\q
```
...
El usuario que crearé, se llamará postgres para menos complicaciones, la contraseña que usaré es ‘D****a’.


```sql
CREATE USER MAPPING FOR postgres
SERVER srv_history
OPTIONS (user 'postgres', password 'D****a');
```
